...
class MainActivity : Activity() {
    ...
    private val tripLocationListener = object : LocationListener {
        override fun onLocationChanged(location: Location) {
            ...
            lastTripLocation?.let { val delta = it.distanceTo(location); if (delta > 2f) tripDistanceKm += delta / 1000.0 }
    ...
    override fun onCreate(b: Bundle?) {
        ...
        findViewById<EditText>(R.id.tripOrigin).apply { isFocusable = false; isClickable = true; setOnClickListener { launchLocationPicker("ORIGIN") } }
        findViewById<EditText>(R.id.tripDestination).apply { isFocusable = false; isClickable = true; setOnClickListener { launchLocationPicker("DESTINATION") } }
    ...
    private fun requestBtPermissions() {
        val p = if (android.os.Build.VERSION.SDK_INT >= 31) arrayOf(Manifest.permission.BLUETOOTH_SCAN, Manifest.permission.BLUETOOTH_CONNECT) else arrayOf(Manifest.permission.ACCESS_FINE_LOCATION)
    ...
    private fun scanClassic() {
        ...
        classic.clear(); adapter.cancelDiscovery(); adapter.bondedDevices.forEach { classic[it.address] = it }
        if (adapter.startDiscovery()) {
            votolStatus.text = "SCANNING…"
            log.text = "Classic discovery active. Select a VOTOL device below."
            handler.postDelayed({ if (adapter.isDiscovering) adapter.cancelDiscovery(); if (votolStatus.text == "SCANNING…") votolStatus.text = "SELECT DEVICE" }, 12000)
        }
        renderClassic()
    }
    ...
    private fun renderClassic() {
        ...
        votolDevices.setOnClickListener { showClassicPicker() }
    }

    private fun showClassicPicker() {
        val devices = classic.values.toList(); if (devices.isEmpty()) return
        val labels = devices.map { "${it.name ?: "Unknown"}\n${it.address}" }.toTypedArray()
        android.app.AlertDialog.Builder(this).setTitle("Pilih VOTOL Controller").setSingleChoiceItems(labels, -1) { dialog, which -> connectVotol(devices[which]); dialog.dismiss() }.setNegativeButton("BATAL", null).show()
    }

    private fun connectVotol(d: BluetoothDevice) {
        Thread {
            runCatching {
                adapter.cancelDiscovery(); runOnUiThread { votolStatus.text = "CONNECTING…" }
                val s = d.createRfcommSocketToServiceRecord(SPP_UUID); s.connect(); votolSocket = s
                runOnUiThread { votolStatus.text = "CONNECTED"; votolStatus.setTextColor(0xFF45D6A3.toInt()); log.text = "VOTOL SPP connected: ${d.name ?: d.address}." }
                readClassic(s.inputStream)
            }.onFailure { e -> runOnUiThread { votolStatus.text = "ERROR"; log.text = "VOTOL: ${e.message}" } }
        }.start()
    }
    ...
    private fun readClassic(input: InputStream) {
        ...
        while (votolSocket?.isConnected == true) {
            val n = runCatching { input.read(buf) }.getOrDefault(-1); if (n <= 0) break
    ...
    private val bleCallback = object : ScanCallback() {
        override fun onScanResult(t: Int, r: ScanResult) { val d = r.device; ble[d.address] = d; runOnUiThread { renderBle() } }
        override fun onScanFailed(e: Int) { runOnUiThread { jkStatus.text = "SCAN ERROR $e"; log.text = "BLE scan failed: $e" } }
    }

    private fun scanBle() {
        if (!adapter.isEnabled) { startActivity(Intent(BluetoothAdapter.ACTION_REQUEST_ENABLE)); return }
        ble.clear(); scanner = adapter.bluetoothLeScanner; scanner?.stopScan(bleCallback); scanner?.startScan(bleCallback)
        jkStatus.text = "SCANNING…"; log.text = "BLE scan active. Select the JK BMS device you want to connect."
        handler.postDelayed({ scanner?.stopScan(bleCallback); if (jkStatus.text == "SCANNING…") jkStatus.text = "SELECT DEVICE" }, 10000)
        renderBle()
    }
    ...
    private fun showJkPicker() {
        ...
        if (devices.isEmpty()) { Toast.makeText(this, "Belum ada device BLE ditemukan", Toast.LENGTH_SHORT).show(); return }
        val labels = devices.map { "${it.name ?: "Unnamed BLE device"}\n${it.address}" }.toTypedArray()
        val selected = devices.indexOfFirst { it.address == jkConnectedAddress }
        android.app.AlertDialog.Builder(this).setTitle("Pilih JK BMS").setSingleChoiceItems(labels, selected) { dialog, which -> connectJk(devices[which]); dialog.dismiss() }.setNegativeButton("BATAL", null).show()
    }

    private fun connectJk(d: BluetoothDevice) {
        runCatching { jkGatt?.disconnect(); jkGatt?.close() }; jkGatt = null; jkConnectedAddress = d.address; jkRxFrameBuffer.clear()
        jkStatus.text = "CONNECTING…"; log.text = "JK BMS BLE connecting to ${d.name ?: d.address}"; jkGatt = d.connectGatt(this, false, gattCallback)
    }
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(g: BluetoothGatt, status: Int, newState: Int) {
            runOnUiThread {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    jkStatus.text = "CONNECTED"; jkStatus.setTextColor(0xFF45D6A3.toInt()); log.text = "JK BMS BLE connected to ${g.device.name ?: g.device.address}. Discovering services…"; g.discoverServices()
                } else { if (jkGatt === g) jkGatt = null; jkStatus.text = "DISCONNECTED"; jkStatus.setTextColor(0xFFFFB86B.toInt()) }
            }
        }
        override fun onServicesDiscovered(g: BluetoothGatt, status: Int) {
            ...
            handler.postDelayed({ sendJkCommand(g, 0x96) }, 1200); handler.postDelayed({ sendJkCommand(g, 0x97) }, 2200)
        }
        override fun onCharacteristicChanged(g: BluetoothGatt, c: BluetoothGattCharacteristic) {
            val data = c.value ?: return; appendJkRxChunk(data)
    ...
    private fun sendJkCommand(g: BluetoothGatt, command: Int) {
        if (g !== jkGatt) return
        val service = g.getService(JK_SERVICE_UUID) ?: return; val characteristic = service.getCharacteristic(JK_WRITE_UUID) ?: return
        val frame = ByteArray(20); frame[0] = 0xAA.toByte(); frame[1] = 0x55.toByte(); frame[2] = 0x90.toByte(); frame[3] = 0xEB.toByte(); frame[4] = command.toByte(); frame[16] = (jkCommandCounter and 0xFF).toByte(); jkCommandCounter = (jkCommandCounter + 1) and 0xFF
        var crc = 0; for (i in 0..18) crc = (crc + (frame[i].toInt() and 0xFF)) and 0xFF; frame[19] = crc.toByte()
        characteristic.writeType = BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE; characteristic.value = frame; val ok = g.writeCharacteristic(characteristic)
    ...
    private fun appendJkRxChunk(value: ByteArray) {
        ...
        while (true) {
            var start = -1
            for (i in 0..jkRxFrameBuffer.size - 4) if (jkRxFrameBuffer[i] == 0x55.toByte() && jkRxFrameBuffer[i + 1] == 0xAA.toByte() && jkRxFrameBuffer[i + 2] == 0xEB.toByte() && jkRxFrameBuffer[i + 3] == 0x90.toByte()) { start = i; break }
            if (start < 0) { if (jkRxFrameBuffer.size > 3) { val keep = jkRxFrameBuffer.takeLast(3); jkRxFrameBuffer.clear(); jkRxFrameBuffer.addAll(keep) }; return }
            if (start > 0) repeat(start) { jkRxFrameBuffer.removeAt(0) }
            if (jkRxFrameBuffer.size < 300) return
            val frame = jkRxFrameBuffer.take(300).toByteArray(); var crc = 0; for (i in 0 until 299) crc = (crc + (frame[i].toInt() and 0xFF)) and 0xFF
    ...
    private fun parseJkRuntimeFrame(frame: ByteArray) {
        ...
        val data = JkBmsData(cellCount, cells, resistances, i32(frame, 150) * 0.001f, i32(frame, 158) * 0.001f, u32(frame, 154) * 0.001f, i16(frame, 162) * 0.1f, i16(frame, 164) * 0.1f, i16(frame, 144) * 0.1f, frame[173].toInt() and 0xFF, u32(frame, 174) * 0.001f, u32(frame, 178) * 0.001f, u32(frame, 182).toInt(), u32(frame, 186) * 0.001f, frame[190].toInt() and 0xFF, i16(frame, 170) * 0.001f, (frame[172].toInt() and 0xFF) != 0, (frame[198].toInt() and 0xFF) != 0, (frame[199].toInt() and 0xFF) != 0, (frame[200].toInt() and 0xFF) != 0, (frame[201].toInt() and 0xFF) != 0, 0, 0L, "LFP", 0)
    ...
    private fun updateJkDashboard(data: JkBmsData) {
        ...
        findViewById<TextView>(R.id.bmsCurrentValue).text = "%.3f A".format(data.current)
        findViewById<TextView>(R.id.bmsPowerValue).text = "%.0f W".format(data.power)
        findViewById<TextView>(R.id.bmsDeltaValue).text = "%.3f V".format(delta)
        findViewById<TextView>(R.id.bmsTempValue).text = "%.1f °C".format(data.temp1)
        findViewById<TextView>(R.id.systemTempValues).text = "Controller     -- °C\n\nMotor              -- °C\n\nBattery            %.1f °C".format(data.temp1)
        val estimateView = findViewById<TextView>(R.id.bmsChargeEstimate)
        val amps = kotlin.math.abs(data.current)
        if (data.fullAh > 0.1f && amps > 0.1f) {
            val hours = data.fullAh / amps
            val totalMinutes = (hours * 60.0).toInt()
            estimateView.text = "ESTIMASI CHARGE 0–100%    ${formatChargeTime(totalMinutes)}"
        } else {
            estimateView.text = "ESTIMASI CHARGE 0–100%    --"
        }
    ...
    private fun startRealTrip() {
        ...
        locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 1000L, 1f, tripLocationListener); tripHandler.post(tripTimer)
    }

    private fun stopRealTrip() {
        tripRunning = false; if (::locationManager.isInitialized) locationManager.removeUpdates(tripLocationListener); tripHandler.removeCallbacks(tripTimer); findViewById<TextView>(R.id.tripDistance).text = String.format("Jarak     %.2f km", tripDistanceKm)
    }
    ...
    private fun launchLocationPicker(mode: String) { startActivityForResult(Intent(this, LocationPickerActivity::class.java).apply { putExtra("MODE", mode) }, 700) }

    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        super.onActivityResult(requestCode, resultCode, data)
        if (requestCode == 700 && resultCode == RESULT_OK && data != null) { val label = data.getStringExtra("LABEL") ?: return; if (data.getStringExtra("MODE") == "ORIGIN") findViewById<EditText>(R.id.tripOrigin).setText(label) else findViewById<EditText>(R.id.tripDestination).setText(label) }
    }

    private fun disconnectJk() { runCatching { jkGatt?.disconnect(); jkGatt?.close() }; jkGatt = null; jkConnectedAddress = null; jkRxFrameBuffer.clear(); jkStatus.text = "DISCONNECTED"; jkStatus.setTextColor(0xFFFFB86B.toInt()) }

    private fun exportSystemLog() {
        val text = log.text.toString(); if (text.isBlank()) { Toast.makeText(this, "SYSTEM LOG masih kosong", Toast.LENGTH_SHORT).show(); return }
        val fileName = "VotolJKUnified_LOG_${System.currentTimeMillis()}.txt"
        val values = ContentValues().apply { put(android.provider.MediaStore.Downloads.DISPLAY_NAME, fileName); put(android.provider.MediaStore.Downloads.MIME_TYPE, "text/plain"); put(android.provider.MediaStore.Downloads.RELATIVE_PATH, "Download/VotolJKUnified") }
        ...
        runCatching { contentResolver.openOutputStream(uri)?.use { it.write(text.toByteArray(Charsets.UTF_8)) } }.onFailure { contentResolver.delete(uri, null, null); Toast.makeText(this, "Gagal menyimpan log", Toast.LENGTH_SHORT).show(); return }
        Toast.makeText(this, "Log tersimpan di Download/VotolJKUnified", Toast.LENGTH_LONG).show()
    }

    override fun onDestroy() { runCatching { unregisterReceiver(receiver) }; stopRealTrip(); disconnectJk(); disconnectVotol(); super.onDestroy() }
}